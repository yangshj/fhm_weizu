...
<div id="page-content" class="clearfix">
  ...
  <div class="row-fluid">
	...
	<div class="row-fluid">
			...
			<!-- 检索  -->
			<form action="weizu/location/list.do" method="post" name="userForm" id="userForm">
			<table border='0'>
				<tr>
					<td>
			...
			<table id="table_report" class="table table-striped table-bordered table-hover">
				...
				<c:choose>
					<c:when test="${not empty locationList}">
						<c:if test="${QX.cha == 1 }">
						<c:forEach items="${locationList}" var="bean" varStatus="vs">
							<tr>
								<td class='center' style="width: 30px;">
									<label><input type='checkbox' name='ids' value="${bean.id }" id="${bean.id }" alt="${bean.id }"/><span class="lbl"></span></label>
								</td>
								...
								<td>
									<img src="${bean.avatarUrl_46}"  alt="${bean.nickName}" />
									<a href="${bean.avatarUrl_0}" target="_blank">点击看大头像</a>
								</td>
								<td>${bean.createTime}</td>
								<td style="width: 60px;" class="center">
									<c:if test="${bean.STATUS == '0' }"><span class="label label-important arrowed-in">冻结</span></c:if>
									<c:if test="${bean.STATUS == '1' }"><span class="label label-success arrowed">正常</span></c:if>
								</td>
								<td style="width: 30px;" class="center">
									<div class='hidden-phone visible-desktop btn-group'>
										...
										<ul class="dropdown-menu dropdown-icon-only dropdown-light pull-right dropdown-caret dropdown-close">
											<c:if test="${QX.edit == 1 }">
												<li><a style="cursor:pointer;" title="编辑" onclick="editUser('${bean.id }');" class="tooltip-success" data-rel="tooltip" title="" data-placement="left"><span class="green"><i class="icon-edit"></i></span></a></li>
											</c:if>
											<c:choose>
												<c:when test="${bean.userName=='admin'}"></c:when>
												<c:otherwise>
													<c:if test="${QX.del == 1 }">
													<li><a style="cursor:pointer;" title="删除" onclick="delUser('${bean.id }','${bean.userName }');" class="tooltip-error" data-rel="tooltip" title="" data-placement="left"><span class="red"><i class="icon-trash"></i></span> </a></li>
													</c:if>
												</c:otherwise>
											</c:choose>
										</ul>
